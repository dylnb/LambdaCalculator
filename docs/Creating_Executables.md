...
15. Copy `jre180` (located in the `launch4j` folder) and `Lambda Calculator SE.exe` OR `Lambda Calculator TE.exe` into the folder. Make sure that the JRE folder is named exactly `jre180` (this is the path specified in the .xml files from Steps 6 and 7), and more importantly, that its contents are as they should be for a Windows computer. For example, folders `bin` and `lib` should be located directly under `jre180`. If this is not the case, you may accidentally be working with the OSX version of the JRE. 
...
23. Once you've published the new release, go to the `gh-pages` branch in the LambdaCalculatorPublic github repository and open `index.html`. Under `<!-- Download -->`, replace the hyperlinks to link to the newly released versions of the Lambda Calculator.
